...
export const ProductoDetalle = () => {
  ...
  const getInstrumento = async () => {
    const instrumentoSelect: Instrumento = await getOneInstrumento(Number(id));
  ...
  const envioClassName = args?.costoEnvio === "G" ? "envioGratis" : "envioPago";

  const generarPDF = () => {
    window.open("http://localhost:9000/api/pdf/" + id);
  };
